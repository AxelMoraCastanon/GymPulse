...
if(isset($data->first_name) && isset($data->last_name) && isset($data->email) && isset($data->phone_number) && isset($data->location_id) && isset($data->password)){
    ...
    if($stmt->execute([$data->first_name, $data->last_name, $data->email, $data->phone_number, $data->location_id, $hashed_password])){
        echo json_encode(["message" => "Trainer added successfully"]);
    } else {
        echo json_encode(["message" => "Failed to add trainer", "error" => $stmt->errorInfo()]);
    }
} else {
